export const widgetStyles = `
  ...
  * { scrollbar-width: none; -ms-overflow-style: none; box-sizing: border-box; }
  ...
  /* 主卡片 */
  .glass-card { position: relative; z-index: 1; display: flex; flex-direction: row; padding: 28px 32px; border-radius: 34px; contain: layout style; transform: translate3d(0, 0, 0); overflow: visible; will-change: width, height; }
  .glass-background { position: absolute; top: 0; left: 0; right: 0; bottom: 0; z-index: -1; border-radius: inherit; border: 1px solid rgba(255, 255, 255, 0.15); box-shadow: 0 0 0 1px rgba(255, 255, 255, 0.08), inset 0 1px 2px rgba(255, 255, 255, 0.2), 0 30px 60px rgba(0, 0, 0, 0.45); contain: strict; transform: translateZ(0); overflow: hidden; }
  ...
  .import-modal-content { background: rgba(28, 32, 40, 0.88); backdrop-filter: blur(40px); border: 1px solid rgba(255, 255, 255, 0.15); width: 460px; padding: 32px; border-radius: 28px; box-shadow: 0 40px 100px rgba(0,0,0,0.6); color: white; }
  ...
  .import-textarea { height: 140px; resize: none; font-family: "JetBrains Mono", Menlo, monospace; font-size: 0.8rem; }
  .import-input:focus, .import-textarea:focus { border-color: rgba(255,255,255,0.25); background: rgba(0,0,0,0.35); }
  .import-apply-btn { width: 100%; margin-top: 14px; padding: 14px; border-radius: 14px; background: rgba(255, 255, 255, 0.08); border: 1px solid rgba(255, 255, 255, 0.1); color: white; font-weight: 700; cursor: pointer; transition: all 0.3s; }
  ...
  .import-footer { font-size: 0.65rem; opacity: 0.25; text-align: center; margin-top: 10px; }
  ...
  .track { width: 100%; height: 6px; background: rgba(255, 255, 255, 0.08); border-radius: 3px; overflow: hidden; margin: 12px 0 20px 0; }
  ...
  /* 核心修改：恢复 overflow-y 为 auto，以支持超过最高限额数量时的自然滚动 */
  .scroll-area { flex: 1; overflow-y: auto; display: flex; flex-direction: column; padding-bottom: 20px; }
  .secondary-items-container { display: flex; flex-direction: column; gap: 20px; border-left: 1.5px solid rgba(255, 255, 255, 0.1); padding-left: 22px; margin-left: 14px; padding-right: 18px; }
